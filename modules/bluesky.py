...
def bluesky(bluesky_cfg: BlueskyConfig, img: SourceImage):
	log.info('Posting to Bluesky')

	try:
		bs = Client()
		bs.login(
			login = bluesky_cfg['username'],
			password = bluesky_cfg['app_password']
		)
	except Exception:
	...
	try:
		...
		log.success(f'Posted image to Bluesky! Link: https://bsky.app/profile/{bluesky_cfg["username"]}/{url}')
		return True
	except Exception:
